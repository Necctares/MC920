...
#Filtro passa baixa, recebe como parametro 'nome/diretorio da imagem' e 'Raio do circulo central'

def passaBaixa(img, raioDoCirculo):
  fft_img = np.fft.fft2(img)
  fft_img = np.fft.fftshift(fft_img)
  fft_espec = 20*np.log(np.abs(fft_img))
  centro_y, centro_x = int(img.shape[0]/2), int(img.shape[1]/2)
  circ = np.zeros(img.shape)
  circ = cv2.circle(circ, (centro_x, centro_y), raioDoCirculo, 1, -1)
...
if __name__ == "__main__":
    if(len(sys.argv) < 2):
        ...
        fft_img, fft_spec, nucleo = passaBaixa(img, 25)
        ...
        img = cv2.imread(sys.argv[1], cv2.IMREAD_GRAYSCALE)
        fft_img, fft_spec, nucleo = passaBaixa(img, int(sys.argv[2]))
        sys.argv[1] = sys.argv[1].replace('.png', '')
        cv2.imwrite(f'{sys.argv[1]}_filtroPassaBaixaRaio{int(sys.argv[2])}.png', fft_img)
        cv2.imwrite(f'{sys.argv[1]}_espectroFourier.png', fft_spec)
        cv2.imwrite(f'{sys.argv[1]}_nucleoPassaBaixaRaio{int(sys.argv[2])}.png', nucleo)